...
function lsx_landing_page_do_pricing_table( $item ) {
	...
	<div style="margin-bottom: 12px;">
		<?php if ( ! empty( $item['config']['recommended'] ) ) { ?><div id="banner_<?php echo esc_attr( $item['_id'] ); ?>" class="recommended" style="background-color: <?php echo esc_attr( $item['config']['banner_color'] ); ?>;"><strong><?php echo esc_attr( $item['config']['banner_text'] ); ?></strong></div><?php } ?>
		<div class="price_table_container">
			<div class="price_table_heading"><?php echo esc_attr( $item['config']['label'] ); ?></div>
			<div class="price_table_body">
				<div class="price_table_row cost" id="bg_color_<?php echo esc_attr( $item['_id'] ); ?>" style="background-color: <?php echo esc_attr( $item['config']['bg_color'] ); ?>;color: <?php echo esc_attr( $item['config']['text_color'] ); ?>;">
					<strong><?php echo esc_attr( $item['config']['price'] ); ?></strong> <span><?php echo esc_attr( $item['config']['rate'] ); ?></span>
				</div>
				<?php
				if ( ! empty( $item['config']['option'] ) ) {
					foreach ( $item['config']['option'] as $option ) { ?>
					<div class="price_table_row" id="option_row_<?php echo esc_attr( $option['_id'] ); ?>" style="position:relative;">
			...
			<?php if ( 'shortcode' == $item['config']['area_type'] ) {
				echo do_shortcode( $item['config']['signup_text'] );
			} else { ?>
			<a href="#" id="signup_bg_<?php echo esc_attr( $item['_id'] ); ?>" class="price_table_signup btn btn-lg btn-block" style="background-color: <?php echo esc_attr( $item['config']['signup_bg'] ); ?>;color: <?php echo esc_attr( $item['config']['signup_color'] ); ?>;"><?php if ( ! empty( $item['config']['signup_text'] ) ) { ?><?php echo esc_attr( $item['config']['signup_text'] ); ?><?php } else { ?>Sign Up<?php } ?></a>
				<?php } ?>
		</div>
	</div>
	<?php
...
function lsx_landing_pages_remove_legacy_scripts() {
	...
	if ( ! is_object( $screen ) || 'post' != $screen->base || 'lsx_landing_page' !== $screen->post_type ) {
		return;
	}
...
function lsx_landing_pages_custom_column_data( $column, $post_id ) {
	global $post;
	if ( 'shortcode' === $column ) {
		echo '[landing_page id="' . esc_attr( $post_id ) . '"]';
	}
}
